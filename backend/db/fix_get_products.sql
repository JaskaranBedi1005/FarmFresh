-- ============================================================
-- FarmFresh - Fix: Complete Package Body with LEFT JOIN
-- Run this ENTIRE script in Oracle SQL Developer.
--
-- KEY FIX: get_products now uses LEFT JOIN so ALL active
-- products are returned, even if farmer_id has no matching
-- user record. Also adds NVL() fallbacks for farmer fields.
-- ============================================================
...
CREATE OR REPLACE PACKAGE BODY farmfresh_pkg AS
  ...
  BEGIN
    ...
    INSERT INTO products (
      ...
      p_price, p_unit, p_stock, NVL(p_discount, 0), p_image_url, p_tags
    );
  ...
  -- ── GET PRODUCTS (FIXED: LEFT JOIN instead of INNER JOIN) ──────
  ...
  BEGIN
    OPEN v_cursor FOR
      SELECT p.product_id, p.name, p.category_id, p.farmer_id,
             NVL(u.name,       'Unknown Farmer') AS farmer_name,
             NVL(u.location,   'India')          AS farmer_location,
             u.avatar_url                        AS farmer_avatar,
             NVL(u.verified,   0)                AS farmer_verified,
    ...
    IF p_role = 'customer' THEN
      OPEN v_cursor FOR
        SELECT o.order_id, o.order_ref, o.customer_id, o.farmer_id,
               ...
               NVL(uf.name, 'Unknown Farmer') AS farmer_name
        FROM   orders o
